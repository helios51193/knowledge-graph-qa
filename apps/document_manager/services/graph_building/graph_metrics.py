...
def build_graph_metrics(graph):
    ...
    average_degree = round(
        sum(len(adjacency[node_id]) for node_id in node_ids) / node_count, 2
...
def _count_connected_components(node_ids, adjacency):
    visited = set()
    components = 0
...
def _largest_component_size(node_ids, adjacency):
    visited = set()
    largest = 0
...
def _bfs(start_node, adjacency, visited):
